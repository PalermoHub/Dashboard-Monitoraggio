...
function applyFilters() {
    ...
    updateFiltersPopup();
	    ...
	    // AGGIUNGI: Aggiorna statistiche del side panel se aperto
    if (document.getElementById('pattoSidePanel')?.classList.contains('open')) {
        updatePanelStatistics();
        if (document.getElementById('charts-tab-content')?.classList.contains('active')) {
...
function updateMap() {
    ...
    filteredData.forEach(patto => {
        ...
        // ✅ POPUP MODIFICATO - USA SIDE PANEL
        const popupContent = `
            <div class="p-2 max-w-xs">
                ...
                <button 
                    onclick="(function() {
                        console.log('🎯 Click bottone popup per ID: ${patto[idKey]}');
                        if (window.map && window.map.closePopup) {
                            window.map.closePopup();
                        }
                        setTimeout(function() {
                            if (typeof window.showPattoDetails === 'function') {
                                window.showPattoDetails('${patto[idKey]}');
                            } else {
                                console.error('❌ showPattoDetails non disponibile');
                            }
                        }, 100);
                    })()" 
                    class="mt-2 bg-blue-600 text-white px-2 py-1 rounded text-xs hover:bg-blue-700 transition-colors w-full">
                    Vedi dettagli
                </button>
            </div>
        `;
...
actionTd.innerHTML = `
    <button 
        onclick="(function() {
            console.log('🎯 Click tabella per ID: ${item[idKey]}');
            if (typeof window.showPattoDetails === 'function') {
                window.showPattoDetails('${item[idKey]}');
            } else {
                console.error('❌ showPattoDetails non disponibile');
            }
        })()" 
...
function showPattoDetails(pattoId) {
    console.log('🎯 showPattoDetails chiamata per ID:', pattoId);
    
    // Assicurati che il side panel sia pronto
    if (typeof window.showPattoDetails !== 'undefined' && window.showPattoDetails !== showPattoDetails) {
        window.showPattoDetails(pattoId);
    } else {
        // Fallback: apri il side panel direttamente
        if (typeof openSidePanel === 'function') {
            openSidePanel(pattoId);
        } else {
            console.error('❌ Side Panel non disponibile');
        }
    }
}
