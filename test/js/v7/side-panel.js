...
function openSidePanel(pattoId) {
    ...
    // 🔒 CHIUDI statsDatavizPanel se aperto
    const statsPanel = document.getElementById('statsDatavizPanel');
    if (statsPanel && statsPanel.classList.contains('open')) {
        if (typeof window.closeStatsDatavizPanel === 'function') {
            window.closeStatsDatavizPanel();
        } else {
            statsPanel.classList.remove('open');
        }
        debugLog('🔒 Stats panel chiuso prima di aprire side panel', null);
    }
...
function closeSidePanel() {
    ...
    if (panel) {
        ...
        // ✅ RIAPRI STATS PANEL DOPO LA CHIUSURA
        setTimeout(() => {
            if (typeof window.openStatsDatavizPanel === 'function') {
                window.openStatsDatavizPanel();
                debugLog('✅ Stats Panel riaperto dopo chiusura Side Panel', null);
            }
        }, 400); // Aspetta che l'animazione di chiusura finisca
    }
}
...
// ==========================================
window.showPattoDetails = openSidePanel;
window.closeSidePanel = closeSidePanel; // ✅ AGGIUNGI QUESTA RIGA

debugLog('✅ Side Panel caricato correttamente', {
    functionsExported: ['showPattoDetails', 'closeSidePanel'],
    mapReady: isMapReady(),
    dataAvailable: window.allData?.length > 0
});