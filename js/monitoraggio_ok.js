// Variabili globali
        ...
        // Inizializzazione mappa con coordinate precise di Palermo
        function initializeMap() {
            map = L.map('map', {
                maxBounds: PALERMO_BOUNDS,
                maxZoom: 18,
                minZoom: 11,
                zoomControl: true // Abilita i controlli zoom di default di Leaflet
            ...
            L.marker(PALERMO_CENTER, {
                icon: L.divIcon({
                    ...
                    html: '<i data-lucide="landmark" class="h-6 w-6"></i>',
                    iconSize: [24, 24],
                    iconAnchor: [12, 12]
                })
            }).addTo(map)
        ...
        // CORREZIONE: Aggiornamento tabella
        function updateTable() {
            if (!filteredData || filteredData.length === 0) {
                document.getElementById('tableCount').textContent = '0';
                document.getElementById('tableHeader').innerHTML = '<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nessun dato disponibile</th>';
            ...
            filteredData.forEach(item => {
                ...
                orderedKeys.forEach(key => {
                    ...
                    if (key.toLowerCase().includes('stato')) {
                        ...
                        td.innerHTML = `<span style="background-color: ${color}; color: white; padding: 4px 8px; border-radius: 6px; font-size: 12px;">${value}</span>`;
                    } else {
                        // Limita lunghezza testo per celle troppo lunghe
                        if (value.toString().length > 50) {
                            td.innerHTML = `<span title="${value}">${value.toString().substring(0, 47)}...</span>`;
                        } else {
        ...
        // CORREZIONE: Aggiornamento filtri con evidenziazione visiva
        ...
        // NUOVO: Funzione per evidenziare filtri attivi
        function updateFilterAppearance(selectElement, value) {
            if (value && value.trim() !== '') {
                // Filtro attivo
                selectElement.classList.remove('border-gray-300');
                selectElement.classList.add('border-blue-500', 'bg-blue-50', 'ring-2', 'ring-blue-200');
                selectElement.style.fontWeight = '600';
            } else {
                // Filtro inattivo
                selectElement.classList.remove('border-blue-500', 'bg-blue-50', 'ring-2', 'ring-blue-200');
                selectElement.classList.add('border-gray-300');
                selectElement.style.fontWeight = 'normal';
            }
        }

        // CORREZIONE: Applicazione filtri con confronto esatto
        function applyFilters() {
            ...
            filteredData = allData.filter(item => {
                ...
                // CORREZIONE: Confronto esatto invece di includes per evitare falsi positivi
            ...
            filteredData.forEach(patto => {
                ...
                const popupContent = `
                    <div class="p-2 max-w-xs">
                        <h3 class="font-semibold text-sm mb-2">${titolo}</h3>
                        <div class="text-xs space-y-1">
                            ...
                            <p><strong>Stato:</strong> 
                                <span style="background-color: ${color}; color: white; padding: 2px 6px; border-radius: 4px; font-size: 10px;">
                                    ${stato}
                                </span>
                            </p>
                        </div>
                        <button onclick="showPattoDetails('${patto[idKey]}')" class="mt-2 bg-blue-600 text-white px-3 py-1 rounded text-xs hover:bg-blue-700 transition-colors">
                            Vedi dettagli
                        </button>
                    </div>
                `;
        ...
        // CORREZIONE: Aggiornamento statistiche basato sui dati filtrati
        function updateStatistics() {
            const statoKey = Object.keys(allData[0] || {}).find(k => k.toLowerCase().includes('stato'));
            
            // IMPORTANTE: Calcola tutto basandosi sui dati filtrati
        ...
        // NUOVO: Funzione per animare i contatori
        function updateCounterWithAnimation(elementId, newValue) {
            ...
            // Semplice animazione del contatore
            const steps = 10;
        ...
        // Aggiornamento grafico con interattività e etichette migliorato - CORREZIONE PADDING TOP
            ...
            chart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        borderWidth: 0,
                        hoverBackgroundColor: colors.map(color => color + 'CC') // Aggiunge trasparenza al hover
                    }]
                },
                options: {
                    responsive: true,
                    ...
                    layout: {
                        padding: {
                            top: 25 // AGGIUNTO: Padding superiore per i valori sui grafici
                        }
                    },
                    plugins: {
                        legend: {
                        ...
                        chart.data.datasets.forEach(function(dataset, datasetIndex) {
                            ...
                            meta.data.forEach(function(bar, index) {
                                ...
                                if (data > 0) {
                                    ...
                                    const x = bar.x;
                                    const y = bar.y - 8; // CORREZIONE: Aumentato spazio dai valori delle barre
        ...
        // Mostra dettagli patto - CORREZIONE LAYOUT MODALE
        ...
        function setupEventListeners() {
            ...
            // CORREZIONE: Pulisci filtri con reset completo
        ...
        function showNotification(message, type = 'info') {
            ...
            notification.className = `fixed top-4 right-4 px-6 py-3 rounded-lg shadow-lg z-[11000] fade-in ${
        ...
        function switchMapLayer(layer) {
            ...
            document.getElementById('mapStandard').className = layer === 'standard' 
                ? 'w-full text-xs p-2 rounded transition-colors bg-blue-600 text-white'
                : 'w-full text-xs p-2 rounded transition-colors bg-gray-100 text-gray-700 hover:bg-gray-200';
