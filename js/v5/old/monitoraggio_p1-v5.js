...
function initializeMap() {
    ...
    try {
        ...
        // ✅ INIZIALIZZA markersLayer PRIMA di usarlo
        ...
        console.log('Mappa inizializzata con markersLayer:', markersLayer);
...
function updateMap() {
    ...
    filteredData.forEach((patto, index) => {
        ...
        // ✅ CREA IL MARKER SENZA CLASSE PULSE NEL CERCHIO PRINCIPALE
        const marker = L.circleMarker([patto.lat, patto.lng], {
            ...
            opacity: 1,
            fillOpacity: 0.8
            // ❌ RIMOSSO: className: 'map-marker-pulse'
        }).addTo(markersLayer);
        
        // ✅ CREA UN LAYER SEPARATO PER IL PULSE (invisibile ai click)
        const pulseLayer = L.circleMarker([patto.lat, patto.lng], {
            radius: 6,
            fillColor: color,
            color: color,
            weight: 3,
            ...
            interactive: false // ❗ IMPORTANTE: non intercetta click
        }).addTo(markersLayer);
        ...
        // ==========================================
        // GESTIONE CLICK SUL MARKER (SOLO SUL MARKER PRINCIPALE)
        // ==========================================
        ...
        // Bind il popup al marker PRINCIPALE
        ...
        // 🔥 PULSE SEQUENZIALE SUL LAYER SEPARATO
        setTimeout(() => {
            if (pulseLayer._path) {
                pulseLayer._path.classList.add('pulsing');
                setTimeout(() => {
                    if (pulseLayer._path) {
                        pulseLayer._path.classList.remove('pulsing');
                    }
                }, 2000);
            }
        }, index * 300);
...
if (safeAddEventListener('clearFilters', 'click', function() {
    ...
    if (safeAddEventListener('closeModal', 'click', function() {
        ...
        if (miniMapContainer) {
			...
			if (miniMapContainer) {
				miniMapContainer.innerHTML = '';
				miniMapContainer.removeAttribute('class');
				miniMapContainer.removeAttribute('style');
			}
...
function setupAutoUpdate() {
    ...
    setInterval(() => {
        ...
        // Esegui solo una volta al giorno alle 14:45
        if (currentMinute === (14 * 60 + 45) && currentMinute !== lastUpdateCheck) {
            lastUpdateCheck = currentMinute;
            console.log('Auto-aggiornamento dati...');
            loadData();
        }
    }, 60000);
}
...
function initializePulseEffect() {
    ...
    if (!markersLayer) {
        console.error('❌ markersLayer non è inizializzato. Riprovo tra 2 secondi...');
        setTimeout(initializePulseEffect, 2000);
        return;
    }
    
    function triggerMapPulse() {
        console.log('🔴 ATTIVAZIONE PULSE');
        
        // ✅ CONTROLLO MULTIPLO
        if (!markersLayer || typeof markersLayer.eachLayer !== 'function') {
            console.warn('❌ markersLayer non disponibile o non valido');
            return;
        }
        
        try {
            // Applica pulse solo ai layer di pulse (non ai marker principali)
            markersLayer.eachLayer((layer) => {
                // Verifica che sia un circle marker E abbia la classe pulse
                if (layer instanceof L.CircleMarker && 
                    layer.options.className === 'map-marker-pulse' && 
                    layer._path) {
                    
                    // Reset e applica pulse
                    layer._path.classList.remove('pulsing');
                    void layer._path.offsetWidth; // Force reflow
                    layer._path.classList.add('pulsing');
            ...
            console.error('❌ Errore durante il pulse:', error);
        }
    }
    
    // Sequenza di pulse sulla mappa
    const pulseTimings = [2000, 5000, 8000, 11000];
    
    pulseTimings.forEach(timing => {
        setTimeout(() => {
            console.log(`🎯 Pulse sui marker alle ${timing}ms`);
            triggerMapPulse();
        }, timing);
    });
    
    console.log('🎯 Effetto pulse configurato');
}
...
map.on('click', function(e) {
    console.log('🗺️ Click sulla mappa in:', e.latlng);
});

// Verifica che i marker rispondano ai click
setTimeout(() => {
    console.log('🔍 Verifica marker sulla mappa...');
    let clickableMarkers = 0;
    markersLayer.eachLayer((layer) => {
        if (layer instanceof L.CircleMarker && layer.options.interactive !== false) {
            clickableMarkers++;
        }
    });
    console.log(`✅ ${clickableMarkers} marker cliccabili trovati`);
}, 5000);
