...
/**
 * FUNZIONE UNIFICATA PER APPLICARE TUTTI I FILTRI IN MODO CASCATO
 * Questa funzione applica prima i filtri standard, poi la ricerca intelligente sui risultati.
 */
function applyFiltersUnified() {
    console.log('🔍 Applicando filtri unificati in modo cascato...');

    // 1. Ottieni i valori dei filtri attivi
    const filters = {};
    ...
    // Filtro titolo "vecchio stile"
    const titoloField = document.getElementById('filterTitolo');
    filters.titolo = titoloField ? titoloField.value.toLowerCase().trim() : '';
    // Filtro ricerca intelligente
    filters.smartSearch = currentSmartSearchQuery ? currentSmartSearchQuery.trim() : '';
    // Filtro proponente nascosto
    filters.proponenteNascosto = (typeof proponenteFilter !== 'undefined') ? proponenteFilter.trim() : '';

    // 2. Applica prima i FILTRI STANDARD a tutti i dati (allData)
    let intermediateFilteredData = allData.filter(item => {
        // Trova le chiavi nel dataset
        const statoKey = Object.keys(item).find(k => k.toLowerCase().includes('stato'));
        const uplKey = Object.keys(item).find(k => k.toLowerCase().includes('upl'));
        const quartiereKey = Object.keys(item).find(k => k.toLowerCase().includes('quartiere'));
        const circoscrizioneKey = Object.keys(item).find(k => k.toLowerCase().includes('circoscrizione'));
        const proponenteKey = Object.keys(item).find(k => k.toLowerCase().includes('proponente'));
        const titoloKey = Object.keys(item).find(k => k.toLowerCase().includes('titolo') && k.toLowerCase().includes('proposta'));

        // Applica i filtri standard (escluso smart search)
        const statoMatch = !filters.stato || (item[statoKey] && item[statoKey].trim() === filters.stato);
        const uplMatch = !filters.upl || (item[uplKey] && item[uplKey].trim() === filters.upl);
        const quartiereMatch = !filters.quartiere || (item[quartiereKey] && item[quartiereKey].trim() === filters.quartiere);
        const circoscrizioneMatch = !filters.circoscrizione || (item[circoscrizioneKey] && item[circoscrizioneKey].trim() === filters.circoscrizione);
        // Applica il filtro titolo vecchio stile
        const titoloMatch = !filters.titolo || (item[titoloKey] && item[titoloKey].toLowerCase().includes(filters.titolo));
        // Applica il filtro proponente nascosto
        ...
        // Restituisci true se TUTTI i filtri standard corrispondono
        return statoMatch && uplMatch && quartiereMatch && circoscrizioneMatch && titoloMatch && proponenteNascostoMatch;
    });

    console.log(`🔍 Dopo filtri standard: ${intermediateFilteredData.length} elementi da ${allData.length} totali`);

    // 3. Applica la RICERCA INTELLIGENTE solo ai dati già filtrati dagli altri criteri
    if (filters.smartSearch) {
        // Usa la funzione esistente executeSmartSearchOnFilteredData, ma passa intermediateFilteredData
        // Nota: Questa funzione potrebbe aver bisogno di essere adattata per restituire solo i dati corrispondenti
        // o di fornire un modo per ottenere i dati filtrati direttamente.
        // Per ora, assumiamo che esegua il matching interno e restituisca un array di risultati completi.
        // Potrebbe essere necessario rivedere questa parte in base all'implementazione esatta.

        // Opzione A: Se executeSmartSearchOnFilteredData restituisce solo gli elementi corrispondenti
        // filteredData = executeSmartSearchOnFilteredData(filters.smartSearch, intermediateFilteredData);

        // Opzione B: Se executeSmartSearchOnFilteredData ha un comportamento diverso,
        // potrebbe essere necessario reimplementare qui la logica di matching basata su `intermediateFilteredData`.
        // Per esempio, usando una funzione di matching semplice:
        const searchQuery = filters.smartSearch.toLowerCase();
        filteredData = intermediateFilteredData.filter(item => {
            // Crea un testo combinato per la ricerca (simile a quanto fatto in executeSmartSearchOnFilteredData)
            // Questo è un esempio semplificato, adattalo alla logica reale di smart search.
            const combinedText = Object.values(item).join(' ').toLowerCase();
            return combinedText.includes(searchQuery); // O usa la logica più complessa di smart search
        });

        console.log(`🔍 Dopo ricerca intelligente ("${filters.smartSearch}"): ${filteredData.length} elementi da ${intermediateFilteredData.length} filtrati`);
    } else {
        // Se non c'è ricerca intelligente, i dati finali sono quelli filtrati dagli standard
        filteredData = intermediateFilteredData;
    }

    // 4. Aggiorna tutte le viste con i dati finali filtrati
    updateAllViewsUnified();
    // Aggiorna anche il popup dei filtri se necessario
    if (typeof updateFiltersPopup === 'function') {
        updateFiltersPopup();
    }
}

// Assicurati che questa funzione sia esportata/globalmente disponibile se necessario
