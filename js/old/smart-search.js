...
// Fix per conflitti con il sistema filtri esistente
// ==========================================
...
/**
 * Inizializza il sistema di ricerca intelligente - VERSIONE CORRETTA
 */
function initializeSmartSearchIntegrated() {
    console.log('🔗 Inizializzazione ricerca intelligente unificata corretta...');
    ...
    // RIMOSSA integrateWithExistingSystem() che causava conflitti
...
/**
 * Setup degli event listeners - CORRETTO per non sovrascrivere quelli esistenti
 */
function setupSmartSearchEventListeners() {
    ...
    // NON rimuovere i listener esistenti, aggiungili solo se non esistono
    if (!input.hasAttribute('data-smart-search-initialized')) {
        ...
        input.addEventListener('keydown', handleSmartSearchKeydown);
    }
    
    // Pulsante clear
    if (clearButton && !clearButton.hasAttribute('data-smart-search-initialized')) {
        clearButton.setAttribute('data-smart-search-initialized', 'true');
...
/**
 * Esegue la ricerca intelligente - CORRETTO per usare il sistema filtri originale
 */
function performSmartSearchIntegrated(query) {
    ...
    // Prima applica i filtri geografici usando il sistema originale
...
/**
 * NUOVA FUNZIONE: Trigger dei filtri originali invece di sostituirli
 */
function triggerOriginalFilters() {
    // Invece di sostituire filteredData, modifichiamo temporaneamente filterTitolo
    const filterTitolo = document.getElementById('filterTitolo');
    if (filterTitolo && currentSmartSearchQuery) {
        // Salva il valore originale
        const originalValue = filterTitolo.value;
        
        // Imposta temporaneamente il valore della ricerca intelligente
        filterTitolo.value = currentSmartSearchQuery;
        ...
        // Non ripristinare il valore originale per mantenere la ricerca attiva
        // filterTitolo.value = originalValue;
    } else if (!currentSmartSearchQuery) {
        // Se non c'è ricerca attiva, pulisci il campo titolo e applica filtri
        if (filterTitolo) {
            filterTitolo.value = '';
        }
        if (typeof applyFilters === 'function') {
...
function applyGeographicalFilters(data, filters) {
    return data.filter(item => {
        ...
        // RIMOSSO il filtro proponente nascosto che causava problemi
...
function selectSmartSearchSuggestionWithCentering(text, lat, lng) {
    ...
    // USA LA FUNZIONE ORIGINALE invece di applyFiltersIntegrated
    triggerOriginalFilters();
...
function setupTableModalFixed() {
    ...
    // NON sostituire se già inizializzato
    if (!showTableBtn.hasAttribute('data-smart-search-table-initialized')) {
        showTableBtn.setAttribute('data-smart-search-table-initialized', 'true');
        ...
        showTableBtn.addEventListener('click', function(e) {
            ...
            try {
                if (typeof updateTable === 'function') {
                    updateTable(); // USA LA FUNZIONE ORIGINALE
                } else {
                    updateTableFixed();
                }
...
/**
 * Reset completo di tutti i filtri - CORRETTO
 */
function clearAllFiltersIncludingSmart() {
    ...
    // USA LA FUNZIONE ORIGINALE per resettare gli altri filtri
...
function clearSmartSearchCompletely() {
    ...
    // Pulisci anche il campo filterTitolo
    const filterTitolo = document.getElementById('filterTitolo');
    if (filterTitolo) {
        filterTitolo.value = '';
    }
...
function setupAdvancedOptions() {
    ...
    [exactMatch, caseSensitive, showCategory].forEach(option => {
        if (option && !option.hasAttribute('data-smart-search-advanced-initialized')) {
            option.setAttribute('data-smart-search-advanced-initialized', 'true');
...
// Aggiornamento tabella corretto - fallback se la funzione originale non esiste
...
// Auto-inizializzazione CORRETTA - non sovrascrive il sistema esistente
...
console.log('🔗 Smart Search Unificato caricato - versione CORRETTA che non interferisce con i filtri esistenti');