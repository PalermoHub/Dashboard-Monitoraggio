...
// *** CORREZIONE: AGGIORNAMENTO FILTRI CON LOGICA CASCATA COMPLETA ***
function updateFilters() {
    ...
    filterMappings.forEach(({ id, key, isGeographical }) => {
        ...
        // *** CORREZIONE: LOGICA DI CASCATA CHE INCLUDE TUTTI I FILTRI ***
        let dataToFilter = [...allData];
        
        // Per i filtri geografici, applica tutti gli altri filtri attivi (incluso stato)
        if (isGeographical) {
            ...
            // *** NUOVO: Per il filtro stato, applica solo i filtri geografici ***
        ...
        if (uniqueValues.includes(currentValue)) {
            ...
            // Se il valore non è più valido, resetta e notifica
            select.value = '';
...
// *** CORREZIONE: SETUP EVENT LISTENERS CON GESTIONE UNIFORME DI TUTTI I FILTRI ***
function setupEventListeners() {
    ...
    // *** CORREZIONE CRITICA: TUTTI I FILTRI CON LA STESSA LOGICA CASCATA ***
    // Ora TUTTI i filtri (incluso filterStato) hanno la stessa gestione
    ['filterStato', 'filterUpl', 'filterQuartiere', 'filterCircoscrizione'].forEach(id => {
        const element = document.getElementById(id);
        if (element) {
            element.addEventListener('change', function() {
                ...
                if (typeof window.applyFiltersUnified === 'function') {
                    // Se esiste il sistema unificato della ricerca intelligente, usalo
                ...
                // Poi aggiorna i filtri per la cascata - FONDAMENTALE PER LA SINCRONIZZAZIONE
                setTimeout(() => {
                    updateFilters();
                    console.log(`Filtri aggiornati dopo cambio di ${id}`);
                }, 100); // Aumentato a 100ms per maggiore stabilità
            });
            
            console.log(`Event listener configurato per ${id}`);
        } else {
            console.error(`Elemento filtro ${id} non trovato`);
        }
    });
    ...
    // *** CORREZIONE: Pulisci filtri con reset completo - MODIFICATO PER INCLUDERE FILTRO PROPONENTE E POPUP ***
...
// *** FUNZIONE DI VERIFICA E TEST - NUOVA ***
function testFilterSynchronization() {
    console.log('🧪 Test sincronizzazione filtri...');
    
    const filters = ['filterStato', 'filterUpl', 'filterQuartiere', 'filterCircoscrizione'];
    let allFiltersPresent = true;
    
    filters.forEach(id => {
        const element = document.getElementById(id);
        if (!element) {
            console.error(`❌ Filtro ${id} non trovato`);
            allFiltersPresent = false;
        } else {
            console.log(`✅ Filtro ${id} presente con ${element.options.length - 1} opzioni`);
        }
    });
    
    if (allFiltersPresent) {
        console.log('✅ Tutti i filtri sono presenti e dovrebbero essere sincronizzati');
    } else {
        console.error('❌ Alcuni filtri non sono sincronizzati correttamente');
    }
    ...
    requiredFunctions.forEach(funcName => {
        if (typeof window[funcName] === 'function') {
            console.log(`✅ Funzione ${funcName} disponibile`);
        } else {
            console.warn(`⚠️ Funzione ${funcName} non disponibile`);
        }
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Test di sincronizzazione dopo un po' di tempo
    setTimeout(testFilterSynchronization, 2000);
});
